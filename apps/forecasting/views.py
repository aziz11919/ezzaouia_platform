...
VALID_KPIS = {'oil', 'gas', 'water', 'bsw', 'gor', 'prodhours'}
...
def _get_params(request):
    """Validate and extract kpi + periods from query params."""
    kpi = request.GET.get('kpi', 'oil')
    if kpi not in VALID_KPIS:
        return None, None, _json(
            {'error': f'Invalid kpi. Choose from: {", ".join(VALID_KPIS)}'},
            status=400
        )
    try:
        periods = max(1, min(int(request.GET.get('periods', 60)), 120))
    except (ValueError, TypeError):
        periods = 60
    return kpi, periods, None
...
def forecast_field(request):
    ...
    kpi, periods, err = _get_params(request)
    if err:
        return err
