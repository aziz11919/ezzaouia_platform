...
from apps.warehouse.models import FactProduction
...
def _make_item(text, priority="normal"):
    ...
    return {"text": text, "priority": priority, "icon": PRIORITY_ICON[priority]}
...
def generate_morning_suggestions():
    ...
    try:
        recent_qs = FactProduction.objects.filter(
            date__full_date__gte=last_7_start,
            date__full_date__lte=today,
        )
        baseline_qs = FactProduction.objects.filter(
            date__full_date__gte=prev_30_start,
            date__full_date__lte=prev_30_end,
        )

        # BSW/GOR proviennent de DimWellStatus via well_status
        recent_by_well = list(
            recent_qs.values("well__well_code").annotate(
                avg_bopd=Avg("daily_oil"),
                avg_bsw =Avg("well_status__bsw"),
                avg_gor =Avg("well_status__gor"),
            )
        )
        baseline_by_well = list(
            baseline_qs.values("well__well_code").annotate(
                avg_bopd=Avg("daily_oil"),
                avg_bsw =Avg("well_status__bsw"),
                avg_gor =Avg("well_status__gor"),
            )
        )
        recent_field   = recent_qs.aggregate(total_oil=Sum("daily_oil"))
        baseline_field = baseline_qs.aggregate(total_oil=Sum("daily_oil"))

    except Exception:
        return _generic_suggestions()

    baseline_map = {
        row["well__well_code"]: row
        for row in baseline_by_well
        if row.get("well__well_code")
    }
    suggestions = []

    for row in recent_by_well:
        code = row.get("well__well_code")
        if not code:
            continue
